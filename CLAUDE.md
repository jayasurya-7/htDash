...
- **Completed-event ordering and date display.** Completed events are ordered by **`completion_date` descending, with `filed_at` as the fallback** when `completion_date` is absent. This applies to the server-side sort in `api_patient_events` (the `complete` list returned by `GET /api/patients/<homer_id>/events`) and to the client-side Timeline-tab sort in `renderTimelineTab` (`patient_detail.js`). Per-tab cards (Adverse Events, Watch Records, Call Logs, Robot Issues) already sort by `completion_date` client-side. When rendering a completed event, show the **clinical date (`completion_date`) as the primary date** and add a smaller secondary line `Filed: <filed_at>` **only when the two differ on the date portion** (`completion_date[:10] !== filed_at[:10]`) — same-day filings are the common case and the second line would just be noise. Exception: `watch_data_upload` always has `completion_date == filed_at[:16]` by design, so the Filed line is suppressed for that event regardless.
- **Date Rule Framework — completion-field bounds.** Every date input that captures **when an event clinically happened** (completion date, session start/end, visit date, call date, removed date, issue-occurred date, etc.) is bounded by a single declarative rule. The framework is intentionally narrow: it covers single-field clinical-date bounds. Cross-field rules (`session_start`/`session_end` same day, A1/A2 atomic gap) and device-page bounds remain where they are.
  - **Spec file:** `config/date_rules.json`. Phase 1 schema:
    ```json
    {
      "$schema_version": 1,
    ...
    ```
    `not_before` is an **array of patient-field names** — the first defined value on the patient is used. Post-activation events get `activationDate`; pre-activation events (`exp_device_install`, `activation` itself, attempts) fall back to `enrollDate`. `not_after: "today"` is a sentinel resolved to the current date at validation time. The empty `events: {}` is the slot for **Phase 2 per-event overrides** (e.g. `watch_record` chain floor, `adverse_event_followup >= adverse_event.completion_date`, `resolve_robot_issue_visit >= robot_issue_visit`).
  - **Server enforcement:** `utils/date_validation.py` loads the JSON and exports `resolve_default_bounds(patient) -> (min_iso, max_iso)` and `validate_completion_date(patient, value) -> Optional[str]`. Every `complete-event/*` route in `routes/user_management.py` calls the validator before writing — one line per route. Identical error message string is used client-side and server-side.
  ...
  - **Phase 1 scope (shipped):** the universal default rule only. Phase 2 will add per-event overrides; Phase 3 will fold in soft-confirm windows (A1/A2 out-of-window). Devices-page date inputs are not yet on this framework — they remain wired via `/devices/api/device-validation-dates`.
...
Notes attached to an already-completed event, after the fact. Same shape and mechanics as free notes, but stored **on the event entry** and reachable **only from the Timeline tab**. **Status:** ⬜ pending implementation (spec finalised).
...
- **UI:** the Timeline keeps its original rich rows (left, inline details) and adds a notes panel on the right as a centered two-pane group. Clicking a real event selects it (hover highlight) and lazy-loads its role-filtered notes into the right panel with an **Add Note** button → reuses the `note-modal`. Synthetic rows (Enrolled, A0) are not selectable.
...
- **Attachment:** one optional PDF at `note_attachments/<note_id>.pdf` (shared folder; UUID filenames never collide). Downloadable by the note's author (any role) or admin — shown in the expanded Timeline row.
