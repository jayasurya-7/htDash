...
- **Call mode (`call_mode`)** — `patient_call`, `followup_call_d07`, and `followup_call_d21` record whether the call was conducted by **`"audio"`** or **`"video"`**. Required field with no default — the therapist must explicitly choose. Rendered as a two-pill radio pair next to the Duration input. Persisted as `call_mode: "audio" | "video"` on the call entry; the server rejects requests where the value is missing or outside the enum.
